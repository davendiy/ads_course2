...
def wave(maze, start, wall_cell):
    ...
    waveMatrix = [[-1 for i in range(m)] for i in range(n)]

    # створення та ініціалізація sources-матриці
    # такої ж розмірності, що і матриця лабіринту
    sources = [[None for i in range(m)] for i in range(n)]

    q = deque()  # Створюємо чергу
    q.appendleft(start)  # Додаємо у чергу координати стартової клітини
...
if __name__ == "__main__":
    test_maze = readMazeFromFile("maze_input.txt", 7)
    showMaze(test_maze)
    print()

    mazeM, test_source = wave(test_maze, (3, 3), WALL_CELL)

    showWay(test_source, (3, 3), (5, 5), "Wave ")
    wayMatrix = drawWayInMatrix(test_source, (3, 3), (5, 5), "Wave ")
