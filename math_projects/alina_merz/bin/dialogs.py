...
class DialogEnterItem:
    ...
    def _make_widgets(self):
        ...
        self._translator = name_dict(self.pre.data_connector.get_categories())   # словник {Category_Name: Category_id}
...
class DialogChangeItem:
    ...
    def _make_widgets(self):
        ...
        self.nb = ttk.Notebook(self._nb_frame, width=500, height=250)  # створення вкладок
        self.nb.pack()
        ...
        self._categories = name_dict(self.pre.data_connector.get_categories())
    ...
    def _make_product_release(self):
        ...
        tree = ttk.Treeview(_frame, yscrollcommand=scroll.set, show='headings')
        scroll.config(command=tree.yview)
        tree.pack(side=RIGHT)
        tree['columns'] = ROWS_DIALOG
        for el in ROWS_DIALOG:
            tree.column(el, width=120, anchor='center')
            tree.heading(el, text=el, anchor='center')

        items = self.pre.data_connector.find_item(self.default['Name'])
        for el in items:
            tmp = tuple(map(lambda a: el[a], ROWS_DIALOG))
            tree.insert("", 'end', text='', values=tmp)
