...
class MainWindow(Tk):
    ...
    def _make_settings_tab(self, main_frame):
        ...
        scroll.pack(side=RIGHT, fill=Y)
        self._sites_tree['columns'] = SITES_FIELDS
        ...
        scroll.pack(side=RIGHT, fill=Y)
        self._words_tree['columns'] = ['Word']
        self._words_tree.pack(side=RIGHT)

        self._words_tree.column("Word", width=560, anchor='nw')
        self._words_tree.heading('Word', text="Word", anchor='nw')

        self._sites_tree.bind('<Double-1>', self._change_word)
    ...
    def _add_word(self, ev=None):
        ...
        if word:
            ...
            self._words_tree.insert('', 0, text='', values=[word])
    ...
    def _add_category(self, ev=None):
        ...
        if category:
            try:
                database.add_category(category)     # FIXME don't add new category
